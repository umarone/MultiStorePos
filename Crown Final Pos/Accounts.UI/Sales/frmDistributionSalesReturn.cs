            ...
            if (ValidateRows())
            {
                if (ValidateControls())
                {
                    if (ValidateBookPeriod())
                    {
                        ...
                        if (IdVoucher == null || IdVoucher == 0)
                        {
                            for (int k = 0; k < DgvStockSales.Rows.Count; k++)
                            {
                                ...
                                oelProductsProfitLoss.NetSaleAmount = Validation.GetSafeDecimal(DgvStockSales.Rows[k].Cells["colAmount"].Value);
                                oelProductsProfitLoss.SaleCost = Validation.GetSafeDecimal(IManager.GetItemsAvgValueWOT(oelProductsProfitLoss.IdItem.Value)) *
                                                                         Validation.GetSafeDecimal(DgvStockSales.Rows[k].Cells["colQuantity"].Value);
                                oelProductsProfitLoss.Units = Validation.GetSafeDecimal(DgvStockSales.Rows[k].Cells["colQuantity"].Value);
