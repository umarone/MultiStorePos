...
namespace Accounts.UI
{
    public partial class frmPosReturn : Form
    {
        ...
        private void grdProducts_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 5)
            {
                if (grdProducts.Rows[e.RowIndex].Cells["colQuantity"].Value != null && Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colQuantity"].Value) > 0)
                {
                    if (grdProducts.Rows[e.RowIndex].Cells["colUnitPrice"].Value != null)
                    {
                        grdProducts.Rows[e.RowIndex].Cells["colAmount"].Value = Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colUnitPrice"].Value) * Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colQuantity"].Value);
                    }
                }
            }
                    ...
                    if (ValidateBookPeriod())
                    {
                        ...
                        if (IdVoucher == null || IdVoucher == 0)
                        {
                            for (int k = 0; k < grdProducts.Rows.Count; k++)
                            {
                                ...
                                oelProductsProfitLoss.AccountType = "Remove";
                        ...
                        if (IdVoucher == null || IdVoucher == 0)
                        {
                            ...
                            EntityoperationInfo infoResult = manager.InsertSalesReturn(oelVoucher, oelSalesDetailCollection, oelCostOfSalesCollection, oelProductsProfitLossCollection);
        ...
        private void toolStripBtnReturns_Click(object sender, EventArgs e)
        {
            frmfindallvouchers = new frmFindVouchers();
            frmfindallvouchers.VoucherType = VoucherTypes.SalesReturnVoucher.ToString();
        ...
        private void cbxProducts_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar != (char)Keys.Enter && e.KeyChar != (char)Keys.Escape)
            {
                //if (EventCallCounter == 0)
                {
                    ...
                    //frmFindOnlineProducts.Location = new Point(110, 187);
        ...
        private void FillPosInvoiceReturnByNumber()
        {
            #region Variables
            ...
            #endregion
            #region Filling Sales Header And Detail
            List<VoucherDetailEL> ListSales = Manager.GetPosSalesReturnTransactionsByNumber(PosReturnInvoiceNumber.Value, Operations.IdProject, Operations.BookNo);
            if (ListSales.Count > 0)
            {
                ...
                List<VoucherDetailEL> listTransactions = Manager.GetSalesReturnSubTransactions(IdVoucher.Value, Operations.IdProject, Operations.BookNo, ListSales[0].IsNetTransaction.Value);
            ...
            {
                MessageBox.Show("Return Invoice Number Not Found ...");
                ClearControls();
            }
            #endregion
        }
        ...
        void frmfindallvouchers_ExecuteFindVouchersEvent(VouchersEL oelVoucher)
        {
            ...
            PosReturnInvoiceNumber = oelVoucher.VoucherNo;
            FillPosInvoiceReturnByNumber();
