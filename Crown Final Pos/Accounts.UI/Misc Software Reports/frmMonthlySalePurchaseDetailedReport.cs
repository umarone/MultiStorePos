...
namespace Accounts.UI
{
    public partial class frmMonthlySalePurchaseDetailedReport : MetroForm
    {
        ...
        private void FillData()
        {
            ...
            if (ReportType == 1)
            {
                list = PManager.GetMonthlyPurchasesWithDetail(Operations.IdProject, Operations.BookNo, AccountNo, ModeType, StartDate, EndDate);
            }
            else if (ReportType == 2)
            {
                list = PManager.GetMonthlyPurchasesReturnWithDetail(Operations.IdProject, Operations.BookNo, AccountNo, ModeType, StartDate, EndDate);
            }
            else if (ReportType == 3)
            {
                list = SManager.GetMonthlySalesReportWithDetail(Operations.IdProject, Operations.BookNo, AccountNo, ModeType, StartDate, EndDate);
            }
            else
            {
                list = SManager.GetMonthlySalesReturnReportWithDetail(Operations.IdProject, Operations.BookNo, AccountNo, ModeType, StartDate, EndDate);
            }
            if (list.Count > 0)
            {
