...
namespace Accounts.UI
{
    public partial class frmSubHeadExpensesDetail : MetroForm
    {
        ...
        private void LoadData()
        {
            ...
            if (chkIgnore)
            {
                list = manager.GetSubHeadWiseDetailExpenseReport(IdAccount, Operations.IdProject, Operations.BookNo);
            }
            else
            {
                list = manager.GetDateWiseSubHeadWiseDetailExpenseReport(IdAccount, Operations.IdProject, Operations.BookNo, StartDate, EndDate);
            }
            if (list.Count > 0)
            {
