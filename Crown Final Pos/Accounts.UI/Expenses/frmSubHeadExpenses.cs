...
namespace Accounts.UI
{
    public partial class frmSubHeadExpenses : MetroForm
    {
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            {
                list = Manager.GetSubHeadWiseExpenseReport(Operations.IdProject, Operations.BookNo);
            }
            else
            {
                list = Manager.GetDateWiseSubHeadWiseExpenseReport(Operations.IdProject, Operations.BookNo, dtStart.Value, dtEnd.Value);
            }
            if (list.Count > 0)
            {
