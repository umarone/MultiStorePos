...
namespace Accounts.DAL
{
    public class SaleDetailDAL
    {
        ...
        public bool DeleteSalesDetails(Int64 IdVoucherDetail, SqlConnection objConn)
        {
            using (SqlTransaction objTran = objConn.BeginTransaction())
            {
                try
                {
                    SqlCommand cmdDeleteSales = new SqlCommand("[Transactions].[Proc_DeleteSalesDetailById]", objConn);
                    cmdDeleteSales.CommandType = CommandType.StoredProcedure;
                    cmdDeleteSales.Transaction = objTran;
                    cmdDeleteSales.Parameters.Add("@IdDetailVoucher", SqlDbType.BigInt).Value = IdVoucherDetail;
        ...
        public bool InsertProductsProfitLoss(List<VoucherDetailEL> oelSaleCollection, Int64? Identity, SqlConnection objConn, SqlTransaction oTran)
        {
            ...
            for (int i = 0; i < oelSaleCollection.Count; i++)
            {
                ...
                cmdSaleDetail.Parameters.Add(new SqlParameter("@ActionResult", DbType.String)).Value = oelSaleCollection[i].AccountType;
                cmdSaleDetail.ExecuteNonQuery();
                cmdSaleDetail.Parameters.Clear();
            }
            return true;
