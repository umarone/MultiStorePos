...
namespace Accounts.DAL
{
    public class TransactionsDAL
    {
        ...
        public List<TransactionsEL> GetItemsProfitLossStatementByDate(Int64 IdProject, DateTime StartDate, DateTime EndDate, SqlConnection objConn)
        {
            List<TransactionsEL> list = new List<TransactionsEL>();
            using (SqlCommand cmdProftLoss = new SqlCommand("[Transactions].[Proc_GetItemsProfitLossStatement]", objConn))
            {
                cmdProftLoss.CommandType = CommandType.StoredProcedure;
                cmdProftLoss.Parameters.Add("@IdProject", SqlDbType.BigInt).Value = IdProject;
                cmdProftLoss.Parameters.Add("@StartDate", SqlDbType.DateTime).Value = StartDate;
                cmdProftLoss.Parameters.Add("@EndDate", SqlDbType.DateTime).Value = EndDate;
                objReader = cmdProftLoss.ExecuteReader();
                while (objReader.Read())
                {
                    TransactionsEL obj = new TransactionsEL();
                    obj.ItemName = Validation.GetSafeString(objReader["ItemName"]);
                    obj.Qty = Validation.GetSafeDecimal(objReader["Quantity"]);
                    obj.TotalAmount = Validation.GetSafeDecimal(objReader["SaleAmount"]);
                    obj.SaleCost = Validation.GetSafeDecimal(objReader["CostOfGoods"]);
                    obj.NetProfit = Validation.GetSafeDecimal(objReader["Profit"]);
